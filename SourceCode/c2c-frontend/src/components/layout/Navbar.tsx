...
import { Search, ShoppingCart, Menu, User, Bell, LogIn } from 'lucide-react';
            ...
            {isAuthenticated ? (
              <>
                <Link to="/tin-nhan" className="relative p-2 rounded-full hover:bg-gray-100">
                  <Bell size={22} />
                  <span className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">2</span>
                </Link>
                <UserDropdown />
              </>
            ) : (
